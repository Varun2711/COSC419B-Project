...
def get_soccer_net_legibility_results(args, use_filtered=False, filter='sim', exclude_balls=True):
    ...
    # Load filtered images if needed
    if use_filtered:
        if filter == 'sim':
    ...
    # Exclude tracks that correspond to soccer balls if needed.
    if exclude_balls:
        updated_tracklets = []
    ...
    # Helper function to process a single tracklet directory.
